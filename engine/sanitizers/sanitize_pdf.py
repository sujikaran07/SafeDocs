...
def sanitize_pdf(in_path: str | Path, out_path: str | Path):
    ...
    try:
        ...
        # PyPDF2 failed (corrupted/malformed PDF) - fall back to aggressive byte-level cleaning
        print(f"⚠️ PyPDF2 failed: {e}")
        print(f"🔧 Attempting aggressive byte-level sanitization...")
        
        try:
            # Read original bytes
            pdf_bytes = orig_bytes
            removed: List[str] = []
            
            # Aggressive keyword scrubbing (6000+ malware terms)
            cleaned = _scrub_bytes_keywords(pdf_bytes, EXPANDED_TERMS)
            
            # Additional manual removal of dangerous PDF structures
            # Remove JavaScript objects
            cleaned = re.sub(rb'/JavaScript\s*<<[^>]*>>', b'', cleaned)
            cleaned = re.sub(rb'/JS\s*\([^)]*\)', b'', cleaned)
            cleaned = re.sub(rb'/JavaScript', b'/DisabledJS', cleaned)
            removed.append("JavaScript_bytescan")
            
            # Remove OpenAction
            cleaned = re.sub(rb'/OpenAction\s*<<[^>]*>>', b'', cleaned)
            cleaned = re.sub(rb'/OpenAction', b'/DisabledOpenAction', cleaned)
            removed.append("OpenAction_bytescan")
            
            # Remove AA (auto-actions)
            cleaned = re.sub(rb'/AA\s*<<[^>]*>>', b'', cleaned)
            cleaned = re.sub(rb'/AA', b'/DisabledAA', cleaned)
            removed.append("AA_bytescan")
            ...
            if _sha256(cleaned) == orig_sha:
                with open(out_path, "ab") as f:
                    f.write(b"\n% SafeDocs: Aggressive byte-level sanitization applied\n")
            
            print(f"✅ Aggressive sanitization completed! Removed: {removed}")
            
            return {
                "status": "ok",
                "sanitized_file": str(out_path),
                "removed": removed,
                "notes": [f"PyPDF2 failed ({str(e)}), used aggressive byte-level cleaning"],
                "stats": {"fallback": True, "pypdf2_error": str(e)},
            }
        except Exception as e2:
            # Last resort: copy original but mark as failed
            shutil.copy(in_path, out_path)
            print(f"❌ All sanitization methods failed: {e2}")
            return {
                "status": "failed",
                "sanitized_file": str(out_path),
                "removed": [],
                "notes": [f"All methods failed. PyPDF2: {str(e)}, Fallback: {str(e2)}"],
                "stats": {},
                "error": str(e2)
            }
